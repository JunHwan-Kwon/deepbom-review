...
export function decide({ policy, baseline, candidate, diff, baselineEvaluation, candidateEvaluation }) {
  ...
  for (const metric of policy.metrics) {
    ...
    add(`metric_${metric.name}`, thresholdPass && regression <= metric.max_regression ? 'pass' : 'fail', 'Evaluate the fixed absolute threshold and maximum regression.',
      { baseline: before.value, candidate: after.value, unit: metric.unit, threshold: metric.threshold, max_regression: metric.max_regression, regression });
  }
