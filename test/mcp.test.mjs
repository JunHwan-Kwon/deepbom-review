...
export function connect(entry,workspace,command='mcp') {
  ...
  return {request,call:(name,args)=>request('tools/call',{name,arguments:args}),close:async()=>{child.stdin.end();await closed;}};
}
test('stdio MCP initializes, prepares/builds/verifies, bounds paths, and exposes correct write annotations',async t=>{
  const work=await fs.mkdtemp(path.join(os.tmpdir(),'review-mcp-'));t.after(()=>fs.rm(work,{recursive:true,force:true}));
  ...
  const client=connect(path.join(root,'bin/deepbom-review.mjs'),workspace);t.after(()=>client.close());
  ...
  const engine=connect(path.join(root,'bin/deepbom-review.mjs'),workspace,'engine-mcp');t.after(()=>engine.close());
